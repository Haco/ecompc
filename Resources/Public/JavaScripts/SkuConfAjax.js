...
function txEcompcSetOption() {
	$('.ecom-configurator-select-package-option-wrap').on('click', function (e) {
		...
		genericAjaxRequest(t3pid, t3lang, 1407764087, 'setOption', {
			...
			if ( result.showResult ) {
				...
				$('#ecom-configurator-summary-table').html(getConfigurationSummary(result.configurationData[2]));
...
function txEcompcIndex() {
	$('.ecom-configurator-package-select').on('click', function (e) {
		...
		if ( $(this).hasClass('ecom-configurator-package-state-0') || $(this).hasClass('current') )
			return false;
		addAjaxLoader('ecom-configurator-ajax-loader');
		genericAjaxRequest(t3pid, t3lang, 1407764087, 'index', {
...
function getOptionHint(option, pid, lang, cObj) {
	...
	hintBoxContent.html('');
	genericAjaxRequest(pid, lang, 1407764087, 'getOptionHint', {
			option: option,
			cObj: cObj
		}, function(result) {
...
function genericAjaxRequest(pid, language, type, action, arguments, onSuccess) {
	$.ajax({
		...
		data: {
			...
			type: parseInt(type),
			request: {
				controllerName: 'SkuConfiguratorAjaxRequest',
				actionName: action,
				arguments: arguments
			}
		},
	...
	if ( options !== null && options.length ) {
		for ( prop in options ) {
			if ( options.hasOwnProperty(prop) ) {
				var content = "<a data-option=\"" + options[prop].uid + "\" data-option-state=\"" + (options[prop].active ? 1 : 0) + "\" class=\"ecom-configurator-select-package-option-wrap " + (options[prop].disabled ? "disabled" : "enabled") + "\" tabindex=\"" + tabIndex + "\">";
...
function getConfigurationSummary(config) {
	...
	for ( var i=0; i<config.length; i++ ) {
		...
		table += "<tr><td>" + config[i].pkg + "</td><td>" + config[i][0].replace(/([^>\r\n]?)(\r\n|\n\r|\r|\n)/g, '$1<br />$2') + "</td><td>" + (config[i].pkgUid ? "<a data-package=\"" + config[i].pkgUid + "\" class=\"ecom-configurator-package-select\"><i class=\"fa fa-edit\"></i></a>" : "") + "</td></tr>";
	}
	table += "</table>";
	return table;
}
